...
class FolderFlattenerGUI:
    ...
    def _create_widgets(self):
        ...
        # Sources section
        ttk.Label(main_frame, text="Source Files/Folders:").grid(row=0, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))
        
        # Sources listbox with scrollbar
        sources_frame = ttk.Frame(main_frame)
        sources_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        sources_scrollbar = ttk.Scrollbar(sources_frame, orient=tk.VERTICAL, command=self.sources_listbox.yview)
        ...
        ttk.Button(sources_btn_frame, text="Browse Files...", 
                  command=self._browse_files).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(sources_btn_frame, text="Browse Folders...", 
                  command=self._browse_folders).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(sources_btn_frame, text="Clear Sources", 
                  command=self._clear_sources).pack(side=tk.LEFT)
        
        # Destination section
        ttk.Label(main_frame, text="Destination Folder:").grid(row=3, column=0, sticky=tk.W, pady=(10, 5))
        ...
        ttk.Button(dest_frame, text="Choose Destination...", 
                  command=self._choose_destination).grid(row=0, column=1)
        ...
        ttk.Label(main_frame, textvariable=self.progress_var).grid(row=6, column=0, columnspan=2, sticky=tk.W, pady=(10, 5))
        ...
        self.flatten_btn = ttk.Button(btn_frame, text="Flatten & Rename", 
                                     command=self._start_flatten)
